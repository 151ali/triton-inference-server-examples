...
# https://github.com/zldrobit/onnx_tflite_yolov3/blob/master/utils/utils.py
...
def non_max_suppression(prediction, conf_thres=0.5, nms_thres=0.5):
    ...
    for image_i, pred in enumerate(prediction):
        ...
        class_pred = class_pred[i].unsqueeze(1)#.int()
        ...
        pred = torch.cat((class_pred, pred[:, 4:5], pred[:, :4]), 1)
        ...
        det_max = []
        for c in pred[:, 0].unique():
            dc = pred[pred[:, 0] == c]  # select class c
            n = len(dc)
            if n == 1:
